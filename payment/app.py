...
ORDER_URL = os.getenv("ORDER_URL", "http://order:3001")
...
@app.get("/version")
def version():
    return jsonify(service="payment", version=GIT_SHA, status="ok"), 200


@app.get("/ready")
def ready():
    """Payment has no downstream deps for liveness. Readiness is always true
    when the process is alive — the /confirm callback to Order is best-effort."""
    rid = request_id_from(request)
...
def charge():
    ...
    # Confirm back to Order. A failed confirm must NOT fail the charge —
