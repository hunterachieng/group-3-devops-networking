...
# Shared gunicorn config (needed for Prometheus multiprocess metrics cleanup).
COPY psenv/gunicorn.conf.py .

# Prometheus multiprocess mode writes one file per worker here; must exist
# and be writable by the non-root user before gunicorn starts.
