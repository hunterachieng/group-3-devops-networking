...
Responsibilities:
  ...
  - POST /checkout  public entrypoint (the only thing the ALB will route to)
...
init_failures(app, SERVICE_NAME, log, log_event, downstream_url=INVENTORY_URL)
...
@app.get("/version")
def version():
    return jsonify(service="order", version=GIT_SHA, status="ok", marker="gate3a-hands-off-2"), 200
...
def confirm():
    """
    Payment calls this when the charge succeeds. Proves the full round trip,
    logged under the SAME request + order id.
    """
    rid = request_id_from(request)
    oid = order_id_from(request)
